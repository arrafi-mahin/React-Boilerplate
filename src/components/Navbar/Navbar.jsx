...
function Navbar() {
  ...
  return (
    <>
      ...
      <div className="">
        <div className={`${active ? 'left-0' : '-left-screen'} top-0 right-0 bottom-0 fixed z-10 bg-[#000000b4] shadow-lg `}>
          <img
            onClick={() => { setIsActive(false); }}
            className="h-6 absolute right-6 top-6"
            src={cancel}
            alt="sdf"
          />

        </div>
        <ul className={`fixed bg-white font-medium flex flex-col gap-4 z-20 py-20 px-10 bottom-0  top-0 min-w-[50vw] max-w-[80vw] duration-500 ease-in-out ${active ? 'left-0' : 'left-[-100vw]'}`}>
          <li>
            <NavLink className={({ isActive }) => (isActive ? 'text-red-500' : 'text-black')} to="/">Home</NavLink>
          </li>
          <li>
